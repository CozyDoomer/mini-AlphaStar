...
class ScalarEncoder(nn.Module):
    ...
    if use_positional_encoding_for_time:
        ...
        time_encoding_all = L.positional_encoding(
            max_position=max_game_seconds,
    ...
    def __init__(
        ...
        n_action_num=SFS.available_actions,
        n_units_buildings=SFS.unit_counts_bow,
        n_effects=SFS.effects,
        n_upgrade=SFS.upgrade,
        ...
        self.home_race_fc = nn.Linear(
            5, original_32
        )  # with relu, also goto scalar_context
        self.away_race_fc = nn.Linear(
            5, original_32
        )  # with relu, also goto scalar_context
        ...
        # additional features
        self.available_actions_fc = nn.Linear(
            n_action_num, original_64
        )  # with relu, also goto scalar_context
        self.unit_counts_bow_fc = nn.Linear(
            n_units_buildings, original_64
        )  # A bag-of-words unit count from `entity_list`, with relu
        self.mmr_fc = nn.Linear(
            7, original_64
        )  # mmr is from 0 to 6 (by divison by 1000), with relu

        self.units_buildings_fc = nn.Linear(
            n_units_buildings, original_32
        )  # with relu, also goto scalar_context
        self.effects_fc = nn.Linear(
            n_effects, original_32
        )  # with relu, also goto scalar_context
        self.upgrade_fc = nn.Linear(
            n_upgrade, original_32
        )  # with relu, also goto scalar_context. What is the difference with upgrades_fc?

        self.build_order_model_size = 16
        self.before_beginning_build_order = nn.Linear(
            n_units_buildings + SCHP.count_beginning_build_order,
            self.build_order_model_size,
        )  # without relu
        self.beginning_build_order_transformer = Transformer(
            d_model=self.build_order_model_size,
        ...
        self.last_action_type_fc = nn.Linear(
            n_possible_actions, original_128
        )  # with relu
        self.last_repeat_queued_fc = nn.Linear(
            n_possible_values, original_256
        )  # with relu
    ...
    def preprocess_numpy(cls, obs, build_order=None, last_list=None):
        ...
        if cls.use_positional_encoding_for_time:
            ...
            time_encoding_1 = (
                L.unpackbits_for_largenumber(game_loop, num_bits=embedding_size)
                .astype(np.float32)
                .reshape(1, -1)
            )
            ...
            time_encoding_2 = (
                (cls.time_encoding_all[seconds]).astype(np.float32).reshape(1, -1)
            )
            ...
            time_encoding = (
                L.unpackbits_for_largenumber(game_loop, num_bits=SFS.time)
                .astype(np.float32)
                .reshape(1, -1)
            )
        ...
        beginning_build_order = np.zeros(
            (
                1,
        ...
        print(
            "beginning_build_order.shape:", beginning_build_order.shape
        ) if debug else None
        ...
        if build_order is not None:
            ...
            print(
                "sum(beginning_build_order):", np.sum(beginning_build_order).item()
            ) if debug else None
        ...
        if last_list is not None:
            [
                last_delay_value,
                last_action_type_value,
                last_repeat_queued_value,
            ] = last_list
    ...
    def forward(self, scalar_list):
        ...
        x = x.reshape(
            x.shape[0], SCHP.count_beginning_build_order * self.build_order_model_size
        )
...
def test(debug=False):
    ...
    beginning_build_order = torch.randn(
        batch_size,
        SCHP.count_beginning_build_order,
        int(SFS.beginning_build_order / SCHP.count_beginning_build_order),
    )
