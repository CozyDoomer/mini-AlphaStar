...
from alphastarmini.lib.hyper_parameters import \
    MiniStar_Arch_Hyper_Parameters as MAHP
...
class LocationHead(nn.Module):
    ...
    def __init__(
        ...
        self.ds_1 = nn.Conv2d(
            mmc + 4, mmc, kernel_size=1, stride=1, padding=0, bias=True
        )
        self.film_blocks_num = 4

        if not self.use_improved_one:
            self.film_net = FiLM(
                n_resblock=self.film_blocks_num,
                conv_hidden=mmc,
                gate_size=autoregressive_embedding_size,
            )
        else:
            self.film_net_mapskip = FiLMplusMapSkip(
                n_resblock=self.film_blocks_num,
                conv_hidden=mmc,
                gate_size=autoregressive_embedding_size,
            )

        self.us_1 = nn.ConvTranspose2d(
            mmc, int(mmc / 2), kernel_size=4, stride=2, padding=1, bias=True
        )
        self.us_2 = nn.ConvTranspose2d(
            int(mmc / 2), int(mmc / 4), kernel_size=4, stride=2, padding=1, bias=True
        )
        self.us_3 = nn.ConvTranspose2d(
            int(mmc / 4), int(mmc / 8), kernel_size=4, stride=2, padding=1, bias=True
        )
        self.us_4 = nn.ConvTranspose2d(
            int(mmc / 8), int(mmc / 16), kernel_size=4, stride=2, padding=1, bias=True
        )
        self.us_4_original = nn.ConvTranspose2d(
            int(mmc / 8), 1, kernel_size=4, stride=2, padding=1, bias=True
        )

        # note: in mAS, we add a upsampling layer to transfer from 8x8 to 256x256
        self.us_5 = nn.ConvTranspose2d(
            int(mmc / 16), 1, kernel_size=4, stride=2, padding=1, bias=True
        )

        # note: when SCHP.world_size=64, we add a new upsampling layer
        self.us_6 = nn.ConvTranspose2d(
            int(mmc / 4), 1, kernel_size=4, stride=2, padding=1, bias=True
        )
    ...
    def forward(
        self, autoregressive_embedding, action_type, map_skip, target_location=None
    ):
        """
        Inputs:
        ...
        reshape_channels = int(
            AHP.autoregressive_embedding_size / (reshap_size * reshap_size)
        )
        ar_map = autoregressive_embedding.reshape(
            batch_size, -1, reshap_size, reshap_size
        )
        ...
        if not self.use_improved_one:
            ...
            x = self.film_net_mapskip(
                x, gate=autoregressive_embedding, map_skip=map_skip
            )
        ...
        # x shape: (-1, 1, 256, 256)
        target_location_logits = x.reshape(
            batch_size, 1 * self.output_map_size * self.output_map_size
        )
        ...
        print(
            "target_location_logits.shape:", target_location_logits.shape
        ) if debug else None
        ...
        target_location_mask = L.action_involve_targeting_location_mask(
            action_type
        ).bool()
        ...
        if target_location is None:
            ...
            location_id = torch.multinomial(
                target_location_probs, num_samples=1, replacement=True
            )
            ...
            for i, idx in enumerate(location_id):
                ...
                target_location[i] = np.array(
                    [target_location_x.item(), target_location_y.item()]
                )

            del location_id
            target_location = torch.tensor(target_location, device=device).long()
            target_location[no_target_location_mask] = torch.tensor(
                [self.output_map_size - 1, self.output_map_size - 1], device=device
            )

        target_location_logits = target_location_logits.reshape(
            -1, self.output_map_size, self.output_map_size
        )
        target_location_logits = (
            target_location_logits * target_location_mask.float().unsqueeze(-1)
        )
...
class ResBlockFiLM(nn.Module):
    ...
    def __init__(self, filter_size):
        ...
        self.conv1 = nn.Conv2d(
            filter_size, filter_size, kernel_size=[1, 1], stride=1, padding=0
        )
...
def test():
    batch_size = 2
    autoregressive_embedding = torch.randn(
        batch_size, AHP.autoregressive_embedding_size
    )
    ...
    if AHP == MAHP:
        for i in range(5):
            map_skip.append(
                torch.randn(batch_size, AHP.location_head_max_map_channels, 8, 8)
            )
    else:
        for i in range(5):
            map_skip.append(
                torch.randn(batch_size, AHP.location_head_max_map_channels, 16, 16)
            )
    ...
    print(
        "autoregressive_embedding.shape:", autoregressive_embedding.shape
    ) if debug else None

    target_location_logits, target_location = location_head.forward(
        autoregressive_embedding, action_type, map_skip
    )
